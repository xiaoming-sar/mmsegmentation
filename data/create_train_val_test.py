...
def create_splits(target_dir, train_ratio=0.7, val_ratio=0.15, test_ratio=0.15, 
                  input_filename='images_all.txt',
                  output_subdir_name='splits', train_filename='train.txt',
                  val_filename='val.txt', test_filename='test.txt'):
    """
    Reads image names from an input file within a target directory,
    shuffles them, and creates train/val/test split files in a subdirectory.
    ...
    Args:
        ...
        train_ratio (float): The proportion of images for the training set. Defaults to 0.7.
        val_ratio (float): The proportion of images for the validation set. Defaults to 0.15.
        test_ratio (float): The proportion of images for the test set. Defaults to 0.15.
        ...
        test_filename (str): Name of the output test file. Defaults to 'test.txt'.
    ...
    if not math.isclose(train_ratio + val_ratio + test_ratio, 1.0, rel_tol=1e-5):
        print(f"Error: train_ratio ({train_ratio}) + val_ratio ({val_ratio}) + test_ratio ({test_ratio}) must equal 1.0")
        return False
    ...
    test_count = total_images - train_count - val_count  # Remainder goes to test
    ...
    if total_images >= 3:
        ...
        actual_total = train_count + val_count + test_count
        if actual_total > total_images:
            # Reduce from largest
            if train_count >= val_count and train_count >= test_count:
                train_count -= (actual_total - total_images)
            elif val_count >= test_count:
                val_count -= (actual_total - total_images)
            else:
                test_count -= (actual_total - total_images)
    ...
    print(f"Splitting into {len(train_names)} training, {len(val_names)} validation, and {len(test_names)} test images.")
    ...
    try:
        with open(test_file, 'w') as f:
            for name in test_names:
                f.write(name + '\n')
        print(f"Written test image names to: {test_file}")
    except IOError as e:
         print(f"Error writing to {test_file}: {e}. Skipping write for this file.")
         success = False
...
def combine_splits(base_dir, subdirectories_list, final_output_dir,
                   ...
                   train_filename='train.txt', val_filename='val.txt', test_filename='test.txt'):
    """
    Combines train.txt, val.txt, and test.txt files from multiple subdirectories into
    ...
    Args:
        ...
        test_filename (str): Name of the source and target test file. Defaults to 'test.txt'.
    """
    ...
    for subdir_path in subdirectories_list:
        ...
        current_test_file = os.path.join(subdir_path, source_subdir_name, test_filename)
        ...
        try:
            with open(current_test_file, 'r') as f:
                ...
                print(f"  Added {len(names)} entries from {test_filename}")
        except FileNotFoundError:
            print(f"  Warning: {current_test_file} not found. Skipping.")
        except Exception as e:
            print(f"  Error reading {current_test_file}: {e}. Skipping.")

    # --- Shuffle the combined lists ---
    random.shuffle(all_train_names)
    random.shuffle(all_val_names)
    random.shuffle(all_test_names)
    print(f"\nShuffled combined lists. Total train: {len(all_train_names)}, Total val: {len(all_val_names)}, Total test: {len(all_test_names)}")
    ...
    final_test_file = os.path.join(final_splits_dir, test_filename)
    try:
        with open(final_test_file, 'w') as f:
            for name in all_test_names:
                f.write(name + '\n')
        print(f"Written combined test image names to: {final_test_file}")
    except IOError as e:
         print(f"Error writing combined test file {final_test_file}: {e}")
...
if __name__ == "__main__":
    ...
    final_output_base_dir = "/cluster/projects/nn10004k/ml_SeaObject_Data/OASIs_dataset_patch1024_3in1/OASIs_dataset_patch1024_3in1_1024by1024"

    # Configure split ratios (must sum to 1.0)
    train_ratio_config = 0.7
    val_ratio_config = 0.15
    test_ratio_config = 0.15
    ...
    print(f"Split ratios - Train: {train_ratio_config}, Val: {val_ratio_config}, Test: {test_ratio_config}")
    ...
    # for each subdirectory, create image_all.txt if it doesn't exist
    for subdir in subdirectories:
        image_all_file = os.path.join(subdir, 'images_all.txt')
        try:
            img_dir = os.path.join(subdir, 'image')
            if not os.path.isdir(img_dir):
                print(f"Warning: Image directory '{img_dir}' not found. Skipping creation of {image_all_file}.")
                continue
            image_files = [f for f in os.listdir(img_dir) if f.endswith('.png')]
            with open(image_all_file, 'w') as f:
                for img_file in image_files:
                    f.write(img_file[:-4] + '\n')
            print(f"Created {image_all_file} with {len(image_files)} entries.")
        except Exception as e:
            print(f"Error creating {image_all_file}: {e}. Skipping this directory.")
            continue # Skip to next subdir on error
    if not subdirectories:
        ...
        for subdir_path in subdirectories:
            ...
            if create_splits(subdir_path,
                             train_ratio=train_ratio_config,
                             val_ratio=val_ratio_config,
                             test_ratio=test_ratio_config,
