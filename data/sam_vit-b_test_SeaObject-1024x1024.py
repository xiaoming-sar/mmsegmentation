...
model = dict(
    backbone=dict(
        ...
        init_cfg=dict(
            ...
            prefix='backbone',
            type='Pretrained'),
        out_channels=256, # to be consistent with the decoder in_channels
        patch_size=16,
        type='ViTSAM',
        arch = 'base',
        use_abs_pos=True,
    ...
    decode_head=dict(
        ...
        loss_decode=dict(
            type='FocalLoss',
            use_sigmoid=True, # need to use this one for focalLoss
            gamma=2.0,
            alpha=0.5,
            reduction='mean',
            class_weight=[2.5, 1.923, 25.0, 25.0,],
        ),
        # loss_decode=dict(class_weight=[2.5, 1.923, 25.0, 25.0,],
