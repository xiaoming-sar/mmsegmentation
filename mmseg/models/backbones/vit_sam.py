...
class TransformerEncoderLayer(BaseModule):
    ...
    def forward(self, x):
        shortcut = x
        # print(f"Shape entering transformer layers: {x.shape}")
        x = self.ln1(x)
        # Window partition
...
class ViTSAM(BaseModule):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor]:
        # print(f"Input shape to ViTSAM backbone x: {x.shape}")
        B = x.shape[0]
        x, patch_resolution = self.patch_embed(x)
        # print(f"Shape after patch embedding x: {x.shape}")
        # print(f"Patch resolution patch_resolution: {patch_resolution}")
        ...
        outs = []
        # print(f"Shape before transformer layers: {x.shape}")
        for i, layer in enumerate(self.layers):
            x = layer(x) #  x remains [B, H, W, C] with torch.Size([1, 64, 64, 768]) 
            ...
            if i in self.out_indices:
                ...
                # print(f"Output shape appended at index {i}: {outs[-1].shape}")
        return tuple(outs)

# before apply permutation when out_channels < 0
# Output shape appended at index 2: torch.Size([1, 64, 64, 768])
# Output shape appended at index 5: torch.Size([1, 64, 64, 768])
# Output shape appended at index 8: torch.Size([1, 64, 64, 768])
# Output shape appended at index 11: torch.Size([1, 64, 64, 768])
